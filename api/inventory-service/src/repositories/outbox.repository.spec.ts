...
describe('OutboxRepository (Inventory Service)', () => {
  ...
  describe('findUnsentLocked', () => {
    ...
    it('should return locked rows', async () => {
      const rows = [{ id: 'outbox-1', status: OutboxStatus.UNSENT }];
    ...
    it('should respect custom batchSize', async () => {
      ...
      await repository.findUnsentLocked(mockEm, 3);
  ...
  describe('markSent', () => {
    it('should set status to SENT and sentAt', async () => {
      const outbox: any = { id: 'outbox-1', status: OutboxStatus.UNSENT, retryCount: 0, sentAt: null };
  ...
  describe('markFailed', () => {
    it('should increment retryCount', async () => {
      const outbox: any = { id: 'outbox-1', status: OutboxStatus.UNSENT, retryCount: 2 };
      mockEm.save.mockResolvedValue(outbox);

      await repository.markFailed(mockEm, outbox);

      expect(outbox.retryCount).toBe(3);
      expect(outbox.status).toBe(OutboxStatus.UNSENT);
    });

    it('should set status to FAILED at 5 retries', async () => {
      const outbox: any = { id: 'outbox-1', status: OutboxStatus.UNSENT, retryCount: 4 };
      mockEm.save.mockResolvedValue(outbox);
